...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["zone_demand_score"] = (
        df["pickup_zone"].map(lambda z: zone_type_map.get("residential", 1)).fillna(1)
    )
...
class SurgePredictionModel:
    ...
    def prepare(self, df: pd.DataFrame):
        ...
        logger.info(
            f"Features: {X.shape} | Target distribution:\n{y.value_counts().sort_index()}"
        )
    ...
    def predict(self, features: dict) -> dict:
        ...
        return {
            ...
            "probabilities": {
                SURGE_LABELS[i]: round(float(p), 3) for i, p in enumerate(pred_proba)
            },
        }
