...
class RideAnomalyDetector:
    ...
    def _add_rule_based_flags(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["flag_midnight_surge"] = (
            (df["hour"].isin([2, 3, 4])) & (df["surge_multiplier"] > 2.5)
        ).astype(int)
    ...
    def train(self, df: pd.DataFrame):
        ...
        logger.success(
            f"Anomaly detector trained on {len(X):,} samples | Threshold: {self.threshold:.3f}"
        )
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Combined flag
        df["is_anomaly"] = (
            (df["is_ml_anomaly"] == 1) | (df["is_rule_anomaly"] == 1)
        ).astype(int)
        ...
        logger.info(
            f"Detected {anomaly_count} anomalies ({anomaly_count/len(df)*100:.1f}%) in {len(df)} records"
        )
        return df
    ...
    def get_anomaly_summary(self, df: pd.DataFrame) -> dict:
        ...
        return {
            ...
            "top_suspicious_zones": anomalies["pickup_zone"]
            .value_counts()
            .head(5)
            .to_dict(),
        }
...
class ZoneAnomalyDetector:
    ...
    def build_baselines(self, zone_hourly: pd.DataFrame):
        ...
        for (zone, hour, dow), group in zone_hourly.groupby(
            ["pickup_zone", "event_hour", "event_day_of_week"]
        ):
        ...
        logger.info(
            f"Built baselines for {len(self.baselines)} zone-hour-dow combinations"
        )
        ...
        for _, row in current_demand.iterrows():
            ...
            if baseline:
                ...
                is_spike = (
                    ...
                    or row["total_rides"] > baseline["q3"] + 1.5 * iqr
                )
            else:
                z_score = 0
                is_spike = False
            ...
            results.append(
                {
                    **row.to_dict(),
                    ...
                    "spike_severity": (
                        "high" if z_score > 4 else "medium" if z_score > 3 else "normal"
                    ),
                }
            )
