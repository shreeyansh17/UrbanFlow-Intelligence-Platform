...
TOPICS = [
    ...
    NewTopic(
        name="rides-dlq", num_partitions=1, replication_factor=1
    ),  # Dead Letter Queue
...
class RideProducer:
    ...
    def on_send_success(self, record_metadata):
        ...
        if self.sent_count % 1000 == 0:
            logger.info(
                f"[RIDES] Sent {self.sent_count:,} messages to {record_metadata.topic}"
            )
    ...
    def on_send_error(self, excp):
        ...
        # Send to DLQ
        self.producer.send(
            "rides-dlq", value={"error": str(excp), "ts": datetime.now().isoformat()}
        )
...
class OrderProducer:
    ...
    def run(self, events_per_second: int = 8):
        ...
        for event in self.generator.stream_events(events_per_second):
            self.producer.send(
                "orders-stream", key=event["order_id"], value=event
            ).add_callback(self.on_send_success).add_errback(self.on_send_error)
