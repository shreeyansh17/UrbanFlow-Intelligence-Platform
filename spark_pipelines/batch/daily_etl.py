...
class DataQualityChecker:
    ...
    def check(self, df: DataFrame, name: str) -> DataFrame:
        ...
        failed_rows = (
            df.filter((F.col("total_amount") < 0) | (F.col("timestamp").isNull()))
            if "total_amount" in df.columns
            else self.spark.createDataFrame([], df.schema)
        )
        ...
        self.results.append(
            {
                ...
                "total": total,
                "bad_rows": bad_count,
                "pass_rate": round((total - bad_count) / total * 100, 2),
            }
        )
        return clean_df
...
class RideETL:
    ...
    def extract(self, input_path: str) -> DataFrame:
        ...
        df = (
            self.spark.read.parquet(input_path)
    ...
    def transform(self, df: DataFrame) -> dict:
        ...
        df = df.withColumn(
            "final_fare",
            F.when(F.col("final_fare") < 0, 0).otherwise(F.col("final_fare")),
        )
        df = df.withColumn(
            "surge_multiplier",
            F.when(F.col("surge_multiplier") < 1.0, 1.0).otherwise(
        ...
        # Feature engineering
        df = df.withColumn(
            "fare_per_km", F.round(F.col("final_fare") / F.col("distance_km"), 2)
        )
        df = df.withColumn(
            "speed_kmh",
            F.round(F.col("distance_km") / (F.col("duration_minutes") / 60), 1),
        )
        ...
        zone_hourly = (
            clean_df.filter(~F.col("is_cancelled"))
            .groupBy("event_date", "event_hour", "pickup_zone")
            .agg(
        ...
        # ── Driver performance ─────────────────────────────────────
        driver_perf = (
            clean_df.filter(F.col("driver_id").isNotNull())
            .groupBy("driver_id", "event_date")
            .agg(
...
class OrderETL:
    ...
    def transform(self, df: DataFrame) -> dict:
        ...
        # Derived columns
        df = df.withColumn(
            "avg_item_value", F.round(F.col("subtotal") / F.col("item_count"), 2)
        )
...
if __name__ == "__main__":
    ...
    run_batch_pipeline(
        args.rides_path, args.orders_path, args.output_dir, args.snowflake
    )
