...
def create_spark():
    return (
        SparkSession.builder.appName("UrbanPulse_Streaming")
...
def process_ride_stream(spark: SparkSession):
    ...
    zone_window = (
        rides.withWatermark("event_ts", "10 minutes")
        .groupBy(F.window("event_ts", "5 minutes"), "pickup_zone")
        .agg(
    ...
    def write_to_pg(batch_df, batch_id):
        if batch_df.count() > 0:
            batch_df.withColumn("batch_id", F.lit(batch_id)).write.format(
                "jdbc"
            ).options(**pg_options).option("dbtable", "live_zone_rides").mode(
                "append"
            ).save()
            logger.info(f"Batch {batch_id}: wrote {batch_df.count()} zone aggregations")

    query = (
        zone_window.writeStream.outputMode("update")
...
def process_order_stream(spark: SparkSession):
    ...
    # ── 5-min GMV per zone ────────────────────────────────────────
    zone_gmv = (
        orders.withWatermark("event_ts", "10 minutes")
        .groupBy(F.window("event_ts", "5 minutes"), "delivery_zone")
        .agg(
    ...
    debug_query = (
        zone_gmv.writeStream.outputMode("update")
        .format("console")
