...
def generate_historical_data(
    uber_records: int = 100_000, zomato_records: int = 80_000, days_back: int = 90
):
    ...
    zomato_records_data = zomato_gen.generate_batch(
    ...
    def uber_stream():
        for event in uber_gen.stream_events(events_per_second=uber_rate):
            if producer:
                ...
                logger.debug(
                    f"[UBER] {event['event_type']} | {event['vehicle_type']} | ₹{event['final_fare']}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", choices=["historical", "stream"], default="historical"
    )
