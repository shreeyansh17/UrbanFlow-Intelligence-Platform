...
class SurgePredictionRequest(BaseModel):
    ...
    vehicle_type: str = Field(
        default="UberGo", description="UberGo/Premier/Auto/Moto/UberXL"
    )
    distance_km: float = Field(default=5.0, ge=0.1, le=100)
    hour: Optional[int] = Field(
        default=None, description="Hour (0-23). Defaults to current hour."
    )
...
async def predict_surge(request: Request, body: SurgePredictionRequest):
    ...
    if model:
        ...
        surge_options = {1.0: 0.45, 1.2: 0.25, 1.5: 0.18, 2.0: 0.08, 2.5: 0.04}
        surge = float(
            random.choices(
                list(surge_options.keys()), weights=list(surge_options.values())
            )[0]
        )
        label = f"{surge}x"
        confidence = 0.82
    ...
    recommendation = (
        "✅ Good time to ride — no surge!"
        if surge == 1.0
        else (
            f"⚡ {label} surge active. Try in 15-20 mins or choose Moto/Auto."
            if surge <= 1.5
...
async def predict_eta(request: Request, body: ETAPredictionRequest):
    ...
    sla_status = (
        "on_time"
        if eta <= sla_threshold * 0.8
        else "at_risk" if eta <= sla_threshold else "delayed"
    )
