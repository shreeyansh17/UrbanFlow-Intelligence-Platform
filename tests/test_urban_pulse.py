...
class TestUberGenerator:
    def setup_method(self):
        from uber_generator import DriverPool, UberRideGenerator
    ...
    def test_gps_coordinates(self):
        ...
        for e in events:
            # Mumbai bounding box
            assert (
                18.8 <= e["pickup_lat"] <= 19.4
            ), f"Lat out of Mumbai range: {e['pickup_lat']}"
            assert (
                72.7 <= e["pickup_lon"] <= 73.2
            ), f"Lon out of Mumbai range: {e['pickup_lon']}"


class TestZomatoGenerator:
    def setup_method(self):
        from zomato_generator import RestaurantPool, ZomatoOrderGenerator
...
class TestSurgeModel:
    ...
    def test_model_predict_structure(self):
        ...
        X, y = make_classification(
            n_samples=500, n_features=17, n_classes=5, n_informative=10, random_state=42
        )
        model.model = xgb.XGBClassifier(
            n_estimators=10, use_label_encoder=False, eval_metric="mlogloss"
        )
        model.model.fit(X, y)
    ...
    def test_rule_based_flags(self):
        ...
        test_data = pd.DataFrame(
            [
                ...
                {
                    "timestamp": "2024-01-15 08:00:00",
                    "distance_km": 0.01,  # anomaly: zero distance
                    "duration_minutes": 20,
                    "final_fare": 5000.0,  # anomaly: high fare
